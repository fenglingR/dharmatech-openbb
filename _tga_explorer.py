...
if st.sidebar.checkbox('Filter', value=False):
    categories = st.sidebar.multiselect(
        label='transaction_catg',
        options=df['transaction_catg'].unique()
    )
    df = df[df['transaction_catg'].isin(categories)]

if not st.sidebar.checkbox('Deposits', value=True):
    df = df.query('transaction_type != "Deposits"')

if not st.sidebar.checkbox('Withdrawals', value=True):
    df = df.query('transaction_type != "Withdrawals"')

if not st.sidebar.checkbox('Public debt', value=False):
